...
export default async function editRoom(
  ...
  const location = rawData.location?.trim() || null;
  ...
  if (!name) {
    return { ok: false, error: "VALIDATION", message: "Room name is required" };
  }
  ...
  try {
    ...
    if (e instanceof PrismaClientKnownRequestError) {
      ...
      if (e.code === "P2002") {
        return { ok: false, error: "CONFLICT", message: "Room already exists" };
      }
    }
